...
定期レビューのとき（[PB-00003](00003-run-review-cycle.md) の冒頭で使う）、文書を大量に追加した直後、または「この決定は何に支えられているか」「この根拠はどこで使われているか」を確認したいとき。
...
2. [GRAPH.md](../GRAPH.md) の `## レビュー信号` を上から読む。種別ごとに扱いを決める
   ...
   - **入口のない手順** — 発見されにくいだけで壊れてはいない。頻繁に使うものだけ [PB-00009](00009-add-skill.md) で skill を足す
   ...
   警告が 0 件で安定した種別は [PB-00011](00011-promote-check.md) でエラーに昇格させる。
...
- `has no evidence anchor` → 決定の前に [PB-00001](00001-add-evidence.md) で evidence を書き、`related` と `## 根拠` の両方に入れる
...
- [EVID-00014](../evidence/00014-reference-graph-from-metadata.md)
