...
番号は自分で数えず、機械に聞く。main と開いている全ブランチを走査する（[ADR-00018](../adr/00018-id-allocation.md)）。
...
競合したファイルを種類で分ける。**種類ごとに解決方法が決まっているので、中身を読んで悩まない**（[ADR-00016](../adr/00016-shrink-conflict-surface.md)）。
...
報告は 2 等級で、振り直す側は等級で決まる（[ADR-00018](../adr/00018-id-allocation.md)）。
...
積み上げ PR（PR の base を別の PR にする）は ID 衝突の温床になる。親がマージされても子は main に届かないため、その間に第三の PR が同じ番号を取る。[REV-00006](../reviews/00006-lifecycle-self-review.md) の実例を読むこと。

## 関連

- [ADR-00016](../adr/00016-shrink-conflict-surface.md) — 競合面を減らす決定
- [EVID-00021](../evidence/00021-shared-ledgers-and-serial-ids-collide.md) — 競合の内訳と先行事例
- [PB-00007](00007-rebuild-index.md) / [PB-00010](00010-review-with-graph.md)
