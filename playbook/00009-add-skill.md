...
2. `.agents/skills/<name>/` を作る。`<name>` は英小文字・数字・ハイフンのみ
3. [templates/skill.md](../templates/skill.md) を `.agents/skills/<name>/SKILL.md` にコピーする
4. Frontmatter を埋める
...
6. Claude Code 用の鏡を作る（[ADR-00011](../adr/00011-cross-tool-agent-integration.md)）

   ```bash
   ln -s "../../.agents/skills/<name>" ".claude/skills/<name>"
   ```

7. 対応する playbook 側の `## 関連` に skill へのリンクを足し、`last_reviewed` を今日にする
8. 生成物を再生成する（グラフ → 索引）

## 検証

- `bash .github/scripts/build-index.sh --check` が PASSED（`name` とフォルダ名の一致、playbook 参照の存在、Claude 鏡の symlink を検査する）
...
- `missing symlink` / `symlink points to` → `.claude/skills/<name>` を作り直す。相対リンク先は `../../.agents/skills/<name>`
- skill が発火しない → 手順を増やすのではなく `description` のトリガー語を具体化する（[EVID-00012](../evidence/00012-skills-are-progressive-disclosure.md)）
