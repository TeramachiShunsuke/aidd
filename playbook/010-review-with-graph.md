...
2. [GRAPH.md](../GRAPH.md) の `## レビュー信号` を上から読む。種別ごとに扱いを決める
   ...
   - **根拠なしの決定** — [PB-001](001-add-evidence.md) で evidence を先に書き、ADR の `## 根拠` に錨を足す
   - **手順のない決定** — 手順化できるなら [PB-002](002-write-adr.md) 側ではなく playbook を追加する。できないなら放置してよい（すべての決定が手順になるわけではない）
   - **入口のない手順** — 発見されにくいだけで壊れてはいない。頻繁に使うものだけ [PB-009](009-add-skill.md) で skill を足す
   - **草案に乗る決定 / 廃止を参照** — 根拠の status を確定させるか、後継へ錨を張り替える
   - **孤立** — 参照も被参照もない。統合するか削除を検討する
3. `## ハブ` を見る。被参照が多い文書は変更の影響範囲が大きい。ここを触る PR は影響先を PR 説明に列挙する
