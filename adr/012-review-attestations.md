...
鮮度（ADR-004）・frozen 不変（ADR-003）・reviews 追記専用（ADR-005）の 3 規則は、単独では正しいが組み合わせると出口のない状態を作る。[EVID-016](../evidence/016-lifecycle-rules-deadlock.md) が commit `c86ecd1` で機械的に確認した。
...
- [EVID-016](../evidence/016-lifecycle-rules-deadlock.md): 3 規則のデッドロックと期限 2026-11-07 の機械的確認
- [EVID-003](../evidence/003-doc-drift-is-regression.md): 放置された文書は回帰であり、鮮度検査そのものは維持すべき

## 結果・トレードオフ

- 利点: frozen 文書を改変せずにレビュー可能になり、ADR-003 と ADR-004 が両立する
...
- [ADR-004](004-staleness-policy.md): 決定 2（90 日）と対象範囲を本 ADR が改訂する。日付同期（決定 1）と `workflow_dispatch`（決定 3）は維持
- [ADR-003](003-frozen-immutability.md) / [ADR-005](005-reviews-append-only.md): どちらも改変しない。本 ADR は検査の適用範囲だけを変える
- [PB-003](../playbook/003-run-review-cycle.md) / [PB-005](../playbook/005-fix-staleness-ci.md)
