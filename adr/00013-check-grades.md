...
[ADR-00010](00010-knowledge-graph-layers.md) で構造グラフを CI に載せたが、落とすのは 4 つだけで、決定と根拠の紐づきに関する検査はすべて警告のままだった。[EVID-00017](../evidence/00017-warnings-do-not-ratchet.md) の集計では紐づきの充足率が 100% である一方、それを保つ機械的な力がなく、さらに本文の `## 根拠` と Frontmatter の `related` のズレが 11 件中 7 件あった。

機械的なレビュー品質を上げる手として、意味グラフ（LLM）の採用も検討した。しかし [EVID-00013](../evidence/00013-graphify-needs-llm-for-docs.md) のとおり出力が非決定的で、ゲートには使えない。上げるべきは構造層である。
...
3 を満たせないため、**frozen 文書は検査 5 の対象外**とする（[ADR-00003](00003-frozen-immutability.md) の不変性が優先する）。対象外にした件数は `GRAPH.md` に出す。
...
意味グラフを CI に入れない方針は [ADR-00010](00010-knowledge-graph-layers.md) のまま変えない。ただし使い方を 1 つ足す。**LLM は検査ではなく提案に使い、採用した関係は `related` に書いて構造層へ焼き込む。**

- LLM が「ADR-00010 は EVID-00009 にも乗っているのでは」と示す
- 人が妥当性を判断する
...
- [EVID-00017](../evidence/00017-warnings-do-not-ratchet.md): 紐づきは 100% 充足だが警告のみで守られておらず、本文と `related` のズレが 7/11 で存在した
- [EVID-00014](../evidence/00014-reference-graph-from-metadata.md): 明示メタデータからの検査は決定的で、破損を実際に検出できる
- [EVID-00013](../evidence/00013-graphify-needs-llm-for-docs.md): LLM 由来のグラフは非決定的で、ゲートには使えない
- [EVID-00010](../evidence/00010-handwritten-index-rots.md): 検査されない規約は守られなくなる
- [ADR-00003](00003-frozen-immutability.md): frozen 文書は改変できないため、新しい検査を無条件に適用できない

## 結果・トレードオフ

- 利点: 「決定が根拠に紐づいているか」が運用の心がけではなく CI の条件になる
- 利点: 本文とメタデータのズレが生まれた時点で落ちるため、グラフの被参照数・ハブ・影響範囲が実態とずれない
- 利点: 昇格基準が明文化され、警告をいつ上げるかの判断（[OQ-00008](../ledger/open-questions.md)）が再現可能になる
- 代償: ADR を書くとき、本文の `## 根拠` に ID を足したら `related` にも足す手間が増える。CI が具体的な差分を示すため機械的に直せるが、往復は増える
- 代償: frozen 文書が検査の穴として残る。ADR-00002 のズレ（EVID-00003）は後継 ADR を作るまで直せない
