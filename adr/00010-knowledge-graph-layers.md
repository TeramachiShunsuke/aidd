...
- Graphify のような LLM ベースのグラフ（docs の意味抽出経路）は、CI にも既定の運用にも入れない。なお Graphify のコード経路（tree-sitter による決定的な AST 抽出。LLM なし）は意味層ではなく構造層の性質を持ち、案件リポでの文脈の絞り込みに使える（扱いは ADR-00027。本リポジトリの CI には入れない）
