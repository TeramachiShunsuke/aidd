...
1. **ループの形**: 受け入れ例 1 行 → 落ちる外側テスト 1 つ → 内側を red / green / refactor → 全体を 1 コマンドの品質ゲートに通す（[PB-00013](../playbook/00013-start-tdd-from-examples.md)）
2. **品質ゲートの構成**: 整形の検査 → 静的検査（lint と、あれば型）→ 単体テスト → 外側（受け入れ）テスト、の順で 1 コマンド（例: `make check`）にまとめる。どれか 1 つでも落ちれば失敗
