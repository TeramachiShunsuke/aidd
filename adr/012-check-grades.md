...
[ADR-010](010-knowledge-graph-layers.md) で構造グラフを CI に載せたが、落とすのは 4 つだけで、決定と根拠の紐づきに関する検査はすべて警告のままだった。[EVID-016](../evidence/016-warnings-do-not-ratchet.md) の集計では紐づきの充足率が 100% である一方、それを保つ機械的な力がなく、さらに本文の `## 根拠` と Frontmatter の `related` のズレが 11 件中 7 件あった。
...
- [EVID-016](../evidence/016-warnings-do-not-ratchet.md): 紐づきは 100% 充足だが警告のみで守られておらず、本文と `related` のズレが 7/11 で存在した
