...
種別の判定は 2 つの問いで行う: (1) 出力の正誤を自動検査が決めるか（決める → 機械的 / 生成）、(2) 検査が区別しない選択肢が複数あるか（ある → 判断）。両方に当たる場合は判断を優先する。

### 2. 昇格 — 自動検査の失敗だけが引き金

- **自動検査**（1 コマンドの品質ゲート。外側テストを含むこと — [ADR-00026](00026-fix-loop-shape-let-projects-pick-toolchains.md) §1）で落ちたら、**検査の失敗出力を添えて同じ階層・同じ effort で 1 回**再試行する。再び落ちたら 1 つ上の階層（effort は種別の既定のまま）に渡す。`L` でも落ちたら人に戻す。1 タスクの自動試行は**最大 4 回**（S→S→M→M、または M→M→L→L）で、それを超えたら人に戻す
...
- **閉じている** = 影響範囲が、その Story の受け入れ例の行と契約ファイルの中に収まり、並走する Story の影響範囲と交わらないこと。Story 起票時に責任者が判定する（[ADR-00025](00025-control-work-units-commits-prs.md) §4）
...
PR 本文（[templates/project-pr.md](../templates/project-pr.md) の「モデル」節、**必須・空欄可**）に次の 5 項目を書く: タスク種別、初回の階層と effort、自動検査の初回合否、再試行回数、最終階層。案件で集計し、階層の既定を見直す根拠はこの集計であり、推測で変えない（[OQ-00043](../ledger/open-questions.md)）。トークン数と所要時間は harness が出せる範囲で任意に添える。
