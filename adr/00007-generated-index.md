...
入口が [README.md](../README.md) のディレクトリ表しかないため、エージェントは個々の文書 ID・status・鮮度を知るのにディレクトリを走査する必要がある。手書きの目次を足しても、[EVID-00010](../evidence/00010-handwritten-index-rots.md) のとおり実体と食い違う。
...
6. `INDEX.md` は鮮度検査（[ADR-00004](00004-staleness-policy.md)）の対象ディレクトリに含めない。生成物に `last_reviewed` を持たせると、実体のレビューなしに日付だけが進むため。

## 根拠

- [EVID-00010](../evidence/00010-handwritten-index-rots.md): 導出可能なデータの二重管理はドリフトを生む
- [EVID-00003](../evidence/00003-doc-drift-is-regression.md): 文書ドリフトは回帰であり CI で止める対象
- [ADR-00006](00006-context-tiers.md): Tier を機械可読に一覧する場所が必要

## 結果・トレードオフ

- 利点: 文書の追加・改名・status 変更が索引に必ず反映される（反映しないと CI が赤）
- 利点: エージェントは 1 ファイルで全 ID・status・鮮度・Tier を把握できる
- 代償: 文書を触るたびに再生成コミットが必要になる（[PB-00007](../playbook/00007-rebuild-index.md) で 1 コマンド化）
- 代償: 生成物をリポジトリに置くためレビュー差分が増える。Frontmatter 由来の行のみに出力を絞って抑える

## 関連

- [PB-00007](../playbook/00007-rebuild-index.md)
