...
| ID | 被参照 | 参照 | タイトル |
| --- | --- | --- | --- |
| ADR-006 | 32 | 17 | 文脈を Tier 0〜3 に分け、ロード順を固定する |
| ADR-012 | 29 | 14 | レビュー証跡を文書から分離し、実効レビュー日で鮮度を判定する |
| LEDGER-OQ | 26 | 0 | Open questions |
| ADR-008 | 25 | 16 | SDD の spec 成果物と知識ベースを、双方向の受け渡しで接続する |
...
| EVID-009 | 20 | 7 | 文脈は有限予算であり、常時ロードは劣化を招く |
| EVID-010 | 20 | 9 | 手書き目次は腐るが、生成物なら差分で腐敗を検出できる |
