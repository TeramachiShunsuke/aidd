...
| ID | 被参照 | 参照 | タイトル |
| --- | --- | --- | --- |
| ADR-00008 | 45 | 16 | SDD の spec 成果物と知識ベースを、双方向の受け渡しで接続する |
| ADR-00006 | 39 | 17 | 文脈を Tier 0〜3 に分け、ロード順を固定する |
| ADR-00014 | 37 | 21 | 実装スペックを契約・決定・振る舞いに分け、不可逆な箇所にだけ設計を残す |
| ADR-00019 | 36 | 27 | 働き方の kernel と案件の考え方を別権威にし、evidence は下書きから入る |
| ADR-00020 | 34 | 36 | 認証は git の外の IdP、git は認証情報を使わず、PF は汎用クライアントとする |
| ADR-00017 | 31 | 16 | 機械は事実の記録と検出に限り、status の遷移は人間が行う |
| LEDGER-OQ | 31 | 0 | Open questions |
| ADR-00012 | 29 | 14 | レビュー証跡を文書から分離し、実効レビュー日で鮮度を判定する |
| PB-00008 | 28 | 18 | SDD の spec と知識ベースを橋渡しする |
...
| 草案に乗る決定 | ADR-00014 → ADR-00024 | 根拠が draft のまま |
