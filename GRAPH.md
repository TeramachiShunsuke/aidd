...
| ADR-00019 | 45 | 27 | 働き方の kernel と案件の考え方を別権威にし、evidence は下書きから入る |
| ADR-00014 | 42 | 19 | 実装スペックを契約・決定・振る舞いに分け、不可逆な箇所にだけ設計を残す |
| PB-00013 | 41 | 12 | 受け入れ例から TDD を始める |
| ADR-00006 | 39 | 17 | 文脈を Tier 0〜3 に分け、ロード順を固定する |
| ADR-00020 | 38 | 36 | 認証は git の外の IdP、git は認証情報を使わず、PF は汎用クライアントとする |
| ADR-00010 | 35 | 18 | 知識グラフを構造層と意味層に分け、構造層だけを CI に置く |
| ADR-00017 | 35 | 16 | 機械は事実の記録と検出に限り、status の遷移は人間が行う |
| LEDGER-OQ | 34 | 0 | Open questions |
| PB-00008 | 30 | 18 | SDD の spec と知識ベースを橋渡しする |
