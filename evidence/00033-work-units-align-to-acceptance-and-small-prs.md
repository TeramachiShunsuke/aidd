...
開発の作業単位（Story・コミット・PR）は、受け入れ例を基準に切ると上限が自然に決まる（1 行 = 外側テスト 1 つ、Story = 代表例 1 行と付随する境界・反例の束）。コミットの見出しは機械可読な規約（Conventional Commits）に、PR は小さく、Issue キーでタスク管理（Jira）と結べば、粒度・命名・規模は人の注意ではなく検査で統制できる。
...
- Google のコードレビュー指針 [Small CLs](https://google.github.io/eng-practices/review/developer/small-cls.html) は、小さな変更はレビューが速く・見落としが減り・マージ衝突が減ると述べ、「1 つの自己完結した変更」を単位に挙げている。同指針は「1 つの自己完結した変更」を基準にしつつ、目安として「100 行程度が妥当、1000 行超は大きすぎる（最終的にはレビュアーの判断）」と絶対値も示している。[ADR-00025](../adr/00025-control-work-units-commits-prs.md) の既定 400 行はこの幅の中で、TDD の束（テスト込み）を 1 PR に収める経験則として置いた。
