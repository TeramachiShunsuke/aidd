...
- Cursor 向けインストール（`graphify cursor install`）は `.cursor/rules/graphify.mdc` を `alwaysApply: true` で書き込む。これは全会話に常時注入される設定で、[ADR-00006](../adr/00006-context-tiers.md) の Tier 0 に相当する枠を消費する。
- 一方で有用な性質もある。辺に `EXTRACTED` / `INFERRED` / `AMBIGUOUS` の確信度タグが付き、「読み取った関係」と「推論した関係」を区別できる。これは本リポジトリが evidence と主張を分ける方針（[EVID-00001](00001-agents-need-evidence.md)）と同じ発想である。
...
- [EVID-00014](00014-reference-graph-from-metadata.md)
