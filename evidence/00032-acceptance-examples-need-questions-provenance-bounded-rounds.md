...
- 外部の方法論。Example Mapping（[Matt Wynne, 2015](https://cucumber.io/blog/bdd/example-mapping-introduction/)）は、ルール・例・**質問**を別のカードとして扱い、答えられない質問を赤いカードとして残したまま、セッションを時間（25 分）で区切る。Specification by Example は、[Martin Fowler の bliki](https://martinfowler.com/bliki/SpecificationByExample.html) では「例（テスト）を仕様として使う」考え方として短く整理され、同名の Gojko Adzic の書籍（Manning, 2011）が、例を共同で書き・精緻化し・自動実行して生きた文書にする工程をプロセスパターンとして挙げる。Example Mapping が「欠落は質問として外に出す」「終わりを時間で区切る」点を明示している。
