...
根拠の入口は、人が観測を書く [PB-001](playbook/001-add-evidence.md) だけではない。Slack / 議事録 / Confluence など散在ソースからは、エージェントが `status: draft` の evidence を起こし、人間が観測を確認してから `active` にする（[PB-018](playbook/018-draft-evidence-from-sources.md)）。ログインアカウントを ACL 付きでワークフローに載せる PF は、このリポジトリの外のクライアントである。認証は Okta、git は認証情報を使わない（[ADR-020](adr/020-platform-is-a-client.md)）。
