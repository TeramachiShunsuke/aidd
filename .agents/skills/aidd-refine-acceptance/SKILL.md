...
description: PdO のビジネススペック（要件）を、TDD に渡せる受け入れ条件（代表例・境界・反例・制約）までブラッシュアップする。エージェントが欠落を番号付きの質問にして PdO が決め、固定観点のレビュー→修正を最大 3 巡回す。「受け入れ条件を詰めたい」「要件から受け入れ例を起こす」「PdO の設計をレビューしてテストに渡せる形にする」「受け入れ基準をブラッシュアップ」と言われたときに使う。
metadata:
...
2. 観点 L1〜L6 で指摘を等級付きで出す。P1 は巡内で自分（または開発）が直し、P0 だけ PdO に回す。回答は URL 付きで反映し requirements に書き戻す
