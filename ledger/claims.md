...
- [ ] CLAIM-00037: エージェントのモデル階層と effort はタスク種別（機械的 / 閉じた生成 / 判断・批評 / 人の決定）で既定を決め、昇格は検査の失敗で行う。文脈は決定的なコードグラフの影響範囲に絞り、常時注入しない（草案依存。EVID-00035 / ADR-00027 が `draft`） — evidence:EVID-00035 EVID-00013 adr:ADR-00027
