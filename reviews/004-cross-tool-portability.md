...
- 範囲: skill の置き場所と規範ファイルの配布（[ADR-011](../adr/011-cross-tool-agent-integration.md) の導入時点）

## きっかけ

skills 導入時（[ADR-009](../adr/009-skills-as-playbook-entrypoints.md)）は Cursor だけを前提に `.cursor/skills/` と決めていた。運用対象を Codex と Claude Code に広げるにあたり、[OQ-006](../ledger/open-questions.md)（複製するか）を決着させる必要が生じた。

## 調べたこと

各ツールの公式ドキュメントで、プロジェクトスコープの skill 探索パスと規範ファイル名を確認した（[EVID-015](../evidence/015-agent-tools-read-different-paths.md)）。3 者が共通で読むディレクトリは存在しなかった。
...
Codex と Claude Code の実機での読み込みは確認していない。手元で動かせたのは Cursor のみで、他 2 つは公式ドキュメントの記述に依拠した前提である（[OQ-012](../ledger/open-questions.md)）。Windows で `core.symlinks` が無効な checkout では鏡が機能しない（[OQ-011](../ledger/open-questions.md)）。いずれも実際に各ツールで動かした人が結果を evidence に足すべき箇所である。
